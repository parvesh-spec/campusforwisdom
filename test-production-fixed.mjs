...
const REPLIT_DOMAIN = process.env.REPLIT_DOMAINS?.split(',')[0] || 'example.com';

console.log('Testing Cashfree Production API with HTTPS URLs...');
...
const testCashfreeAPI = async () => {
  try {
    const createOrderRequest = {
      ...
      order_meta: {
        return_url: `https://${REPLIT_DOMAIN}/payment/success`,
        notify_url: `https://${REPLIT_DOMAIN}/api/payments/webhook`,
      },
    ...
    if (response.ok) {
      ...
      console.log('CF Order ID:', data.cf_order_id);
    } else {
